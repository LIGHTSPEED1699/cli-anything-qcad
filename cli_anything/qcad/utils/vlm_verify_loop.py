...
def _dxf_verify(check: VerificationCheck, dxf_path: str) -> bool:
    ...
    try:
        ...
        elif check.task_type == "overlap_check":
            # Check if any added labels overlap with existing text entities
            # by scanning for TEXT entities whose bboxes intersect
            try:
                from cli_anything.qcad.engines.text_value import (
                    _text_bbox, _estimate_text_width,
                )
            except ImportError:
                return True  # can't check, assume pass

            labels = check.expect_present or []
            if not labels:
                return True

            # Build a list of all TEXT/MTEXT entities and their bboxes
            all_texts = []
            for ent in msp:
                if ent.dxftype() in ("TEXT", "MTEXT"):
            ...
            for label in labels:
                ...
                for ent in all_texts:
                    et = ((ent.dxf.text or "").strip().upper()
                          if ent.dxftype() == "TEXT" else
                          (ent.text or "").strip().upper())
                    if et == label_upper:
                        # Check if this label overlaps any other text
                        lx1, ly1, lx2, ly2 = _text_bbox(ent)
                        for other in all_texts:
                            if other is ent:
                                continue
                            ox1, oy1, ox2, oy2 = _text_bbox(other)
                            # Rectangle intersection
                            if (lx1 < ox2 and lx2 > ox1 and
                                ly1 < oy2 and ly2 > oy1):
