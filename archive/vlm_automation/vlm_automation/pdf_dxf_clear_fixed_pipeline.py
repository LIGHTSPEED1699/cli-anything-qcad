...
def run_pdf_to_dxf_clear(pdf_path: Path, dxf_in: Path, dxf_out: Path,
    ...
    for assoc in associations:
        ...
        if not p:
            ...
            continue
        rect = p["rect"]
        bbox = (rect[0] * scale_x + offset_x - margin,
    ...
    # Log
    log = {
        "config": {"margin": margin, "text_match_bonus": 0.5,
                   "min_confidence": 0.30, "max_per_annot": 3},
        "annotations": {"total": len(annots), "polygons": len(polygons),
                        "delete_texts": len(delete_txts), "spare_texts": len(spare_txts)},
        "per_annot": per_annot,
        "cleared": [{"handle": d["handle"], "type": d["etype"],
                     "text": d["text"], "pos": [round(x, 4) for x in d["pos"]],
                     "confidence": round(d["confidence"], 2)} for d in unique_targets],
        "files": {"dxf": str(dxf_out),
                  "dwg": str(dwg_out) if dwg_out.exists() else None},
    }
